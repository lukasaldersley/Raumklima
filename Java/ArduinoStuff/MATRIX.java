...
public class MATRIX implements MouseListener, ActionListener
{
    ...
    public MATRIX(int nr,int hor,int ver)
    {
        ...
        basePanel.setLayout(new GridLayout(1,nr,50,0));//1 bedeutet nur 1 in y richtung,nr bedeutet nr in x richtung, 50 bedeutet 50 px abstand zwischen bl�cken
            ...
            for(int x=0;x<panelX;x++){
                for(int y=0;y<panelY;y++){
                    if(e.getSource()==rechteck[z][x][y]){
                        farbWechsel(z,x,y);
                        window.setSize(0,0);//das hier, zusammen mit window.pack sorgt daf�r dass keine der button fragmente zu sehen sind
                        window.pack();
    ...
    public void log(String inhalt,String fileName) {
        ...
        try {
            writer = new FileWriter(file,true);//true damit an die bestehende datei angeg�ngt wird, sonst w�rde Sie �berschrieben
            writer.write(inhalt);
            writer.flush();
            writer.close();
            ln(fileName);
