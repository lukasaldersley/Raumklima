...
import demo.orsoncharts.LineChart3D2;
...
public class LineChart3DDemo2
extends JFrame {
    public LineChart3DDemo2(String string) {
        super(string);
        this.addWindowListener(new ExitOnClose());
        this.getContentPane().add(LineChart3DDemo2.createDemoPanel());
    }

    public static JPanel createDemoPanel() {
        DemoPanel demoPanel = new DemoPanel(new BorderLayout());
        demoPanel.setPreferredSize(OrsonChartsDemo.DEFAULT_CONTENT_SIZE);
        CategoryDataset3D categoryDataset3D = LineChart3D2.createDataset();
        Chart3D chart3D = LineChart3D2.createChart(categoryDataset3D);
